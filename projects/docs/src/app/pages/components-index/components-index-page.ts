...
const INDEX: ReadonlyArray<IndexGroup> = [
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'mk-barcode-scanner / MkBarcodeScannerDialog', desc: 'Camera QR / barcode reader; decoder loaded on demand.', path: '/components/images' },
      { name: 'MkTabAttention', desc: 'Favicon counter badge + title blink while the tab is hidden.', path: '/components/attention' },
      { name: 'MkNotificationSound', desc: 'Chime / file presets with the autoplay unlock handled.', path: '/components/attention' },
      { name: 'MkSessionExpiry / mk-session-expiry-dialog', desc: 'Warn before a session lapses; extend or sign out.', path: '/components/attention' },
