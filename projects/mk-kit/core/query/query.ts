import { MK_DEFAULT_I18N, type MkI18nStrings } from '../i18n/mk-i18n';
import { mkUniqueId } from '../a11y/unique-id';
import type {
  MkQueryField,
  MkQueryGroup,
  MkQueryNode,
...
export function mkQueryOperatorLabel(op: MkQueryOperator, i18n: MkI18nStrings = MK_DEFAULT_I18N): string {
  const key = `queryOp${op[0].toUpperCase()}${op.slice(1)}` as keyof MkI18nStrings;
  return (i18n[key] as string | undefined) ?? op;
}
...
export function mkQueryToText(
  group: MkQueryGroup,
  fields: readonly MkQueryField[] = [],
  i18n: MkI18nStrings = MK_DEFAULT_I18N,
): string {
  const byKey = new Map(fields.map((f) => [f.key, f]));
